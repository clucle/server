	...
	bool			use_full_checksum)
{
	...
	if (use_full_checksum) {
		...
		compile_time_assert(FIL_PAGE_END_LSN_OLD_CHKSUM % 8 == 0);
...
static byte* buf_page_encrypt(fil_space_t* space, buf_page_t* bpage, byte* s)
{
  ...
  if (!encrypted && !page_compressed)
  {
    ...
    compile_time_assert(FIL_PAGE_FCRC32_KEY_VERSION % 4 == 0);
    compile_time_assert(FIL_PAGE_FILE_FLUSH_LSN_OR_KEY_VERSION % 4 == 2);
    if (full_crc32)
