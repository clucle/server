	...
	mtr_t*			mtr)
{
	...
	if (UNIV_LIKELY_NULL(page_zip)) {
		compile_time_assert(FIL_PAGE_PREV % 8 == 0);
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	if (!recovery) {
		if (block->page.id.page_no() == index->page
			...
			compile_time_assert(!(FIL_PAGE_TYPE % 2));
			memcpy_aligned<2>(FIL_PAGE_TYPE + page,
					  FIL_PAGE_TYPE + temp_page, 2);
			compile_time_assert(!((PAGE_HEADER+PAGE_INSTANT) % 2));
			   ...
			   mtr_t* mtr)
{
	...
	if (prev_page_no != FIL_NULL) {
		...
		compile_time_assert(FIL_PAGE_NEXT % 4 == 0);
		compile_time_assert(FIL_PAGE_OFFSET % 4 == 0);
	...
	if (next_page_no != FIL_NULL) {
		...
		compile_time_assert(FIL_PAGE_PREV % 4 == 0);
		compile_time_assert(FIL_PAGE_OFFSET % 4 == 0);
...
retry:
	...
	if (is_left) {
...
#ifdef UNIV_BTR_DEBUG
		union { byte b[4]; uint32_t u; } fil_page_prev;
#endif /* UNIV_BTR_DEBUG */
		...
		if (merge_page_zip && left_page_no == FIL_NULL) {
			...
			compile_time_assert(FIL_PAGE_PREV % 8 == 0);
#ifdef UNIV_BTR_DEBUG
			memcpy_aligned<4>(fil_page_prev.b,
		...
		if (!orig_succ) {
			...
			if (left_page_no == FIL_NULL) {
				...
				merge_page_zip. */
				ut_a(!memcmp_aligned<4>(fil_page_prev.b,
							merge_page
							+ FIL_PAGE_PREV, 4));
			}
#endif /* UNIV_BTR_DEBUG */
		...
		if (merge_page_zip && left_page_no == FIL_NULL) {
			...
			are X-latched. */
			memcpy_aligned<4>(merge_page + FIL_PAGE_PREV,
					  fil_page_prev.b, 4);
		}
#endif /* UNIV_BTR_DEBUG */
